"""
Penelopa: a compact GPT language model implementation.
References:
...
class Penelopa(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        # forward the Penelopa model itself
    ...
    def from_pretrained(cls, model_type, override_args):
        ...
        # block_size is always 1024 for Penelopa model checkpoints
        # if one wants a lower block_size it has to be done through model surgery
        # later, by calling crop_block_size()

        # create a from-scratch initialized minPenelopa model
        config = PenelopaConfig(block_size=1024, **config_args)
        model = Penelopa(config)
        sd = model.state_dict()
